...
from moviepy.editor import * # or TextClip, concatenate_videoclips
...
intro_duration = 5
intro_text = intro_text = TextClip('Hello world!', fontsize=70, color='white', size=video_clip.size)
intro_text = intro_text.set_duration(intro_duration)
intro_text = intro_text.set_fps(fps)
intro_text = intro_text.set_pos('center') # Not the position of the text but the entire frame of the text
...
watermark_size = 60
watermark_text = TextClip("Fares", fontsize=watermark_size, color='white', align='West', size=(w, watermark_size))
...
final_clip = concatenate_videoclips([intro_text, overlay_clip])# Add to video 
